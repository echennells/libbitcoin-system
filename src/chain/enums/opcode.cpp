...
// Mirror of Bitcoin Core src/script/script.cpp GetOpName(). Fork-independent:
// Core returns full mnemonics for CLTV/CSV/CHECKSIGADD and the (historically
// disabled) opcodes regardless of consensus flags, so active_forks is unused.
std::string opcode_to_bitcoind(opcode value, uint32_t) NOEXCEPT
{
    ...
    switch (value)
    {
        // push value (Core emits bare numbers for small-int opcodes).
        ...
        // splice ops (disabled, but Core still names them).
        ...
        // expansion. nop2/nop3 are enum aliases for CLTV/CSV; Core's GetOpName
        // is fork-independent so these always return the full mnemonic.
        ...
        // Core has an explicit 0xff case (script.cpp).
        case opcode::reserved_255:      return "OP_INVALIDOPCODE";

        // Core's GetOpName has no case for raw data-push bytes (0x01-0x4b) or
        // for reserved_187..reserved_254, so its switch falls through to the
