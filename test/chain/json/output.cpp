...
// Pay-to-public-key: bitcoind classifies as "pubkey" and emits the key-hash
// (P2PKH) address of the embedded key. Vector is the genesis coinbase output.
BOOST_AUTO_TEST_CASE(output__bitcoind__pay_public_key__type_address_asm)
{
    const auto out = to_output(5000000000ull,
    ...
    const auto value = json::value_from(bitcoind(out));
    const auto& output_script = value.as_object().at("scriptPubKey").as_object();
    BOOST_REQUIRE_EQUAL(output_script.at("type").as_string(), "pubkey");
    BOOST_REQUIRE_EQUAL(output_script.at("asm").as_string(),
...
BOOST_AUTO_TEST_CASE(output__bitcoind__witness_key_hash_testnet4__tb_address)
{
    const auto out = to_output(100000ull,
        "0014751e76e8199196d454941c45d1b3a323f1433bd6");
    const auto context = to_address_context(selection::testnet4);
    const auto value = json::value_from(bitcoind(out, context));
    const auto& output_script = value.as_object().at("scriptPubKey").as_object();
    BOOST_REQUIRE_EQUAL(output_script.at("type").as_string(), "witness_v0_keyhash");
    BOOST_REQUIRE_EQUAL(output_script.at("address").as_string(),
