...
// ECH-38/ECH-39/ECH-37/ECH-43: Core-parity (#778) for a real witness coinbase.
// mainnet block 840000 coinbase a0db149ace...272cf (first post-halving block).
// Authoritative raw witness serialization (316 bytes); Core getrawtransaction:
// size=316, vsize=289, weight=1156, with a real non-null witness "hash" (wtxid).
...
BOOST_AUTO_TEST_CASE(transaction__bitcoind__witness_coinbase_840000__core_parity)
{
    // witness=true so the segregated tx carries its witness data.
    ...
    // Coinbase input: has "coinbase", no "txid".
    ...
    // vout0 is P2PKH (ECH-43 type + address, ECH-37 asm).
    const auto& spk0 = vout.at(0u).as_object().at("scriptPubKey").as_object();
    BOOST_REQUIRE_EQUAL(spk0.at("type").as_string(), "pubkeyhash");
    BOOST_REQUIRE_EQUAL(spk0.at("asm").as_string(),
        "OP_DUP OP_HASH160 536ffa992491508dca0354e52f32a3a7a679a53a"
        " OP_EQUALVERIFY OP_CHECKSIG");
    BOOST_REQUIRE_EQUAL(spk0.at("hex").as_string(),
        "76a914536ffa992491508dca0354e52f32a3a7a679a53a88ac");
    BOOST_REQUIRE_EQUAL(spk0.at("address").as_string(),
        "18cBEMRxXHqzWWCxZNtU91F5sbUNKhL5PX");

    // vout1 is OP_RETURN (nulldata, no address).
    const auto& spk1 = vout.at(1u).as_object().at("scriptPubKey").as_object();
    BOOST_REQUIRE_EQUAL(spk1.at("type").as_string(), "nulldata");
    BOOST_REQUIRE(!spk1.contains("address") && !spk1.contains("addresses"));
}
