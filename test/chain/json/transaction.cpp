...
// bitcoind verbose serialization of a witness coinbase (mainnet block 840000),
// checking that "size" is the witness-included total and "hash" is the
// witnessed serialization hash (non-null), matching Core. scriptPubKey parity
...
BOOST_AUTO_TEST_CASE(transaction__bitcoind__witness_coinbase_840000__size_and_witness_hash)
{
    ...
    // Core displays the witnessed serialization hash, not null_hash (the
    // wtxid of a coinbase, which hash(true) returns).
    BOOST_REQUIRE_EQUAL(object.at("hash").as_string(),
        "ba2a2a86efa306684a88b6e0cdb8abf25d9f66e6a851c9b9c67f25640cdacac8");

    // Core "size" is the witness-included total (316), not base (280).
    ...
    // Three outputs (scriptPubKey field parity covered by a separate change).
